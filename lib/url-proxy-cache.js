            ...
            if (mtime && ttl != -1 && (ttl === 0 || ((mtime + ttl) > new Date().getTime()))) {
                ...
                function loadCached() {
                    ...
                    FS.readFile(metaPath, function(err, meta) {
                        ...
                        var storedMeta = false;
                        try {
                            meta = storedMeta = JSON.parse(meta);
                        } catch(err) {
                            fail(err);
                            return;
                        }
                        ...
                        if (meta.status !== 200 && meta.status !== 304) {
                            if (self.options.verbose) console.log("cached " + meta.status + " " + urlInfo.href);
                            deferred.resolve(meta);
                            deferred = null;
                            return;
                        }
                        // If original request was 200 but path no longer exists.
                        if (!pathExists && storedMeta.status === 200) {
                ...
                function makeRequest() {
                        ...
                        if (ttl != -1) {
                            ...
                            PATH.exists(metaPath, function(exists) {
                                ...
                                FS.readFile(metaPath, function(err, data) {
                                    ...
                                    if (self.options.verbose) console.log("http HEAD " + urlInfo.href);
                                    var request = ((urlInfo.protocol==="https:")?HTTPS:HTTP).request({
                                        ...
                                        res.on("end", function() {
                                            if (self.options.verbose) console.log("head " + res.statusCode + " " + urlInfo.href);
                                            ...
                                            if (res.statusCode === 200) {
                                                ...
                                                if (same) {
                                                    ...
                                                    if (options.loadBody === false) {
                                                        if (self.options.verbose) console.log("emit " + existingMeta.status + " " + urlInfo.href);
                                                        wait.reject();
                                                        deferred.resolve(existingMeta);
                                                        return;
                                                    }
                    ...
                    Q.when(done, function() {
                        ...
                        if (self.options.verbose) console.log("http GET " + urlInfo.href);
                        var request = ((urlInfo.protocol==="https:")?HTTPS:HTTP).request({
                            host: urlInfo.host,
                            port: urlInfo.port || ((urlInfo.protocol==="https:")?443:80),
                            path: urlInfo.path,
                            ...
                            if (self.options.verbose) console.log("get " + res.statusCode + " " + urlInfo.href);
                            ...
                            if (res.statusCode !== 200) {
                                writeMeta(function(err) {
                                    ...
                                    if (self.options.verbose) console.log("emit " + meta.status + " " + urlInfo.href);
                                    deferred.resolve(meta);
                                    deferred = null;
                                    writeStream.end();
                                });
