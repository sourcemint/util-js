...
function getProxyAgent(urlInfo) {
    var proxyUrl = (urlInfo.protocol==="https:")?
        (process.env.HTTPS_PROXY || process.env.https_proxy ||
         process.env.HTTP_PROXY || process.env.http_proxy || null):
        (process.env.HTTP_PROXY || process.env.http_proxy || null);
    var proxyUrlInfo = URL.parse(proxyUrl);
    var proxy = { host: proxyUrlInfo.host,
                  port: proxyUrlInfo.port
                };
    var agent;
    if(urlInfo.protocol === "https:") {
        if(proxyUrlInfo.protocol === "https:") {
            agent = TUNNEL.httpsOverHttps({ proxy: proxy });
        } else {
            agent = TUNNEL.httpsOverHttp({ proxy: proxy });
        }
    } else {
        if(proxyUrlInfo.protocol === "https:") {
            agent = TUNNEL.httpOverHttps({ proxy: proxy });
        } else {
            agent = TUNNEL.httpOverHttp({ proxy: proxy });
        }
    }
    return agent;
}
