// TODO: Look into https://github.com/substack/node-optimist to see if it can replace options
//       parsing for sourcemint. But we really need something that configures the parser
//       based on a JSON config.
...
exports.Parser.prototype.act = function (args, options) {
    ...
    options.acted = true;
    if (typeof this._action.call === "function") {
        this._action.call(this, this.parse(args), options);
    } else {
        this._action.act(args, options);
    }
};
...
exports.Parser.prototype.masthead = function (masthead) {
        ...
        function (name, command) {
            ...
            self.print('  \0bold(\0green(' + name + '\0)\0)' + (
                ...
                (
                    ...
                    parser._help
                ) : ((parser._action && parser._action._help) ?
                        ' : ' + parser._action._help : ''
                     )
            ));
        }
    );
